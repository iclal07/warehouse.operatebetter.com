...
async function main() {
    ...
    const user = await prisma.user.create({
        data: {
            ...
            passwordHash: await (0, bcryptjs_1.hash)("demo123", 12),
            firstName: "Mert",
            lastName: "Yılmaz",
        },
    });
    const categories = await Promise.all(categoryNames.map((name, index) => prisma.category.create({
    ...
    const movementTypes = ["IN", "OUT", "TRANSFER", "ADJUSTMENT", "COUNT_CORRECTION"];
